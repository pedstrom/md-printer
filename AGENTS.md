...
- Verification and the commit gate default to macOS. Do not run iOS tests for Mac-only changes. Add `--include-ios` to `scripts/verify.sh` and `scripts/commit_staged.sh` only when the intended change affects the iOS app, mobile support, or shared code used by iOS; unrelated local iOS edits are not a reason to expand a Mac-only check.
