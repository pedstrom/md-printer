...
9. For an app-version change, run `scripts/build-and-run/package_release.sh` from the verified commit with the Developer ID identity and notarytool Keychain profile. Require Apple acceptance, a stapled ticket, a passing Gatekeeper assessment, and a validated universal ZIP before pushing.
10. When Pete has authorized release publication, push the commit and matching version tag, refresh the GitHub release asset, and verify the public download matches the local notarized archive.
