...
- Use the first level-one Markdown heading as the macOS document-window title, falling back to the filename when no H1 exists. Keep document-window chrome minimal: Save PDF and Print only, with no duplicate metadata strip inside the window. Command-S opens Save PDF and Command-P opens Print.
