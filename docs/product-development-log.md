...
- Preserved zoom and the actual visible page-boundary position instead of trusting PDFKit's potentially stale current destination, distributed semantic anchors across the complete viewport, and rejected anchors whose cross-page placement would otherwise clamp to a page top.
